...
- Do not assume opening the USB serial device is non-invasive. A later no-reset raw serial attach correlated with loss of tracker uplinks and then local diagnostic records, while other receiver-side traffic continued. Keep detailed bench evidence private, and treat the root cause as an open investigation until a known-safe serial access method is proven.
